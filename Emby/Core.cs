...
namespace Emby
{
    internal class Core
    {
        ...
        public Core()
        {
            ...
            _bot.MessageUpdated += Bot_MessageUpdated;
            _bot.UserIsTyping += Bot_UserIsTyping;
        ...
        {
            userId = ("<@" + e.User.Id + ">").Replace("<!@", "<@").Replace("<@!", "<@");
        ...
        public void Bot_MessageReceived(object sender, MessageEventArgs e)
        {
            ...
            */
            ...
            else if ((e.Message.RawText.ToLower().StartsWith(".ae") && str.Length == 3 && isUserAdmin) || (e.Message.RawText.ToLower().StartsWith(".ae") && str.Length == 3 && Admins.Contains(("<@" + e.User.Id + ">").Replace("<!@", "<@").Replace("<@!", "<@"))))
            {
                ...
                if (pointsToAdd == 0)
                {
                    if (messagetosend == 0)
                        ...
                        e.Channel.SendMessage("Are you a hacher? Please don't try to break my code!");
                    else if (messagetosend == 2)
                        e.Channel.SendMessage("Please don't make me feel that I'm useless. :cry: ");
                }
                else
                {
            ...
            else if ((e.Message.RawText.ToLower().StartsWith(".re") && str.Length == 3 && isUserAdmin) || (e.Message.RawText.ToLower().StartsWith(".re") && str.Length == 3 && Admins.Contains(("<@" + e.User.Id + ">").Replace("<!@", "<@").Replace("<@!", "<@"))))
            {
                ...
                if (pointsToAdd == 0)
                {
                    if (messagetosend == 0)
                        ...
                        e.Channel.SendMessage("Are you a h#cher? Please don't try to break my code!");
                    else if (messagetosend == 2)
                        e.Channel.SendMessage("Please don't make me feel that I'm useless. :cry: ");
                }
                else
            ...
            else if ((e.Message.RawText.StartsWith(".remove") && isUserAdmin) || (e.Message.RawText.StartsWith(".remove") && Admins.Contains(("<@" + e.User.Id + ">").Replace("<!@", "<@").Replace("<@!", "<@"))))
            {
                userId = str[Convert.ToInt32(Command.Uid)];
                ...
                else if (isUserAdmin || Admins.Contains(("<@" + e.User.Id + ">").Replace("<!@", "<@").Replace("<@!", "<@")))
            ...
            else if (e.Message.RawText.ToLower().StartsWith(".setkickrate") && isUserAdmin)
            {
                try
                {
                    if (Convert.ToInt32(str[1]) > -0.1)
                    {
                        kickrate = Convert.ToInt32(str[1]);
                        e.Channel.SendMessage(e.User.Mention + " Succesfully set the kickrate to " + kickrate + " warnings.");
                    }
                    else if (Convert.ToInt32(str[1]) < 0)
                        e.Channel.SendMessage(e.User.Mention + " Please enter a positive value.");
                }
                catch
                {
                    e.Channel.SendMessage(e.User.Mention + " Please enter a value.");
                }
            }
            ...
            else if ((e.Message.RawText.StartsWith(".warn") && isUserAdmin) || (e.Message.RawText.StartsWith(".warn") && Admins.Contains("@" + e.User.Name + "#" + e.User.Discriminator)))
            {
                ...
                e.User.SendMessage(e.User.Mention + " You have succesfully warned " + str[Convert.ToInt32(Command.Uid)]);
            }
        }
